...
export async function Navbar() {
  ...
  return (
    <nav className="bg-emerald-800 text-white shadow-md">
      ...
      <div className="hidden md:block max-w-7xl mx-auto px-4">
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center space-x-1">
            ...
            <Link href="/messages" className="px-3 py-2 rounded-md text-sm font-medium hover:bg-emerald-700 flex items-center gap-2"><MessageCircle className="w-4 h-4"/> {t('messages')}</Link>
          </div>
      ...
      <MobileNav role={profile?.role} userName={profile?.name} />
    </nav>
  )
}
